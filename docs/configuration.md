...
```cpp
#define ECHOLINK_DISPLAY_BRIGHTNESS 42
#define ECHOLINK_DISPLAY_DIM_BRIGHTNESS 5
...
第一个值控制设备保持某个方向多久后确认方向。沙漏运行时使用 `LIFETIME_HOURGLASS_ACTIVE_IMU_SAMPLE_MS`，暂停且静止时使用 `LIFETIME_HOURGLASS_IDLE_IMU_SAMPLE_MS`，其他 LifeTime 互动使用 `LIFETIME_IMU_SAMPLE_MS`。减小采样周期会提高灵敏度，也会增加功耗。
...
当前沙漏统一使用第一个值，即 120 秒。其余数值保留用于兼容旧配置，不再控制 UI 旋转或实际时长。
...
- MQTT Host、端口、用户名和密码
- MQTT Topic
- MQTT 消息检查频率：网页输入 0.1-60 秒

在 EchoLink 页面长按 `B` 2.5 秒，连接临时热点并访问 `http://192.168.4.1` 配置。信息保存在设备 NVS 中，最多保存 5 个 Wi-Fi Profile。

检查频率只降低 MQTT Task 的空闲唤醒次数。Socket 保持连接，待发语音会立即唤醒任务，Socket 中已有的语音分片也会连续排空。新消息提示可能延迟约设置的秒数；当前 QoS 0 协议不支持断开 Wi-Fi 数分钟后补拉历史消息。
