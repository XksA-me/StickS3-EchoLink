...
运行时 Wi-Fi 与 MQTT 通过设备网页配置。只有无线分组、显示功耗、音频上限和 LifeTime 参数需要修改源码后重新烧录。
...
这里使用 POSIX TZ 语法，`CST-8` 表示 UTC+8。其他地区可使用标准 POSIX 时区字符串。
...
第一个值控制设备保持某个方向多久后启动 Flow；第二个值控制 LifeTime 前台的 IMU 采样周期。减小采样周期会提高灵敏度，也会增加功耗。
