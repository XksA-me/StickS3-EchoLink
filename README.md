...
- 网页可选择 20ms、250ms、1s 或 5s 的 MQTT 消息检查间隔。
- 可选服务器 Recorder，将完整语音保存为 WAV 和 JSONL 元数据。

### LifeTime

- `BATTERY`：大号电量、端电压、粗略续航估算和持久化木鱼计数。
- `HOURGLASS`：根据设备六个稳定放置方向选择时长，翻转后沙漏与倒计时一起换向重启。
...
| --- | --- |
| 通用 | `A` 单击切换下一页；双击切换应用 |
| BATTERY | `B` 木鱼加一；长按 `B` 清零 |
| HOURGLASS | 稳定放置一个方向 1 秒自动开始；换方向后换向并重启；长按 `A` 暂停/继续；长按 `B` 归零 |
