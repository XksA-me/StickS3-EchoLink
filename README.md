...
- **EchoLink**：按住说话、松开发送；支持 ESP-NOW 短距离直连和 Wi-Fi MQTT 跨网络通信。
- **LifeTime**：时钟、专注倒计时、体感沙漏，以及两台设备之间的摇一摇互动。
...
- `NOW`：联网时钟、日期、持久化 Moments 计数。
- `FOCUS`：1/3/5/10/15/30/60 分钟专注倒计时。
- `FLOW`：根据设备六个放置方向自动选择时长并运行流沙动画。
- `TOGETHER`：双摇或按键向另一台设备发送 Ping，对方播放提示音和脉冲动画。

### 功耗策略

- 15 秒无操作自动降低背光，30 秒后关闭显示。
- 黑屏后停止 UI 帧提交，后台通信和定时器继续运行。
- 麦克风与功放仅在录音、播放、提示音期间启用。
- IMU 只在 LifeTime 前台限频采样。
- MQTT 与主循环采用忙时快速、闲时低频调度。
...
设备空闲时同时按住 `A+B` 约 1.2 秒：

| 操作 | 作用 |
| --- | --- |
| `B` 单击 | 在 EchoLink / LifeTime 之间选择 |
| `A` 单击 | 打开选中的应用 |
...
| 操作 | 作用 |
| --- | --- |
| 按住 `A` 说话，松开 | 录音并加入后台发送队列 |
| 有未读消息时短按 `A` | 播放下一条未读语音 |
| `B` 单击 | 切换音量 |
| `B` 双击 | 切换 `LOCAL` / `WIFI` |
| 按住 `B` 2.5 秒 | 开启 5 分钟网络配置入口 |
...
| 通用 | `A` 单击切换下一页 |
| NOW | `B` 加一；长按 `B` 清零 |
| FOCUS | `B` 选择预设；长按 `A` 开始/暂停；长按 `B` 重置 |
| FLOW | 稳定放置一个方向 1 秒自动开始；长按 `A` 暂停；长按 `B` 重置 |
| TOGETHER | 双摇设备或按 `B` 发送；长按 `B` 清空互动计数 |
...
LifeTime 时区、IMU 采样周期和六面沙漏时长位于 [`include/LifeTimeConfig.h`](include/LifeTimeConfig.h)。详细字段见 [固件配置](docs/configuration.md)。
...
| [固件配置](docs/configuration.md) | Group ID、ESP-NOW、亮度、时区和 LifeTime 时长 |
...
<table>
  <tr>
    <td align="center"><img src="docs/assets/wechat-official-placeholder.svg" alt="简说 Python 公众号二维码占位" width="220"><br><strong>公众号：简说 Python</strong></td>
    <td align="center"><img src="docs/assets/wechat-personal-placeholder.svg" alt="个人微信二维码占位" width="220"><br><strong>个人微信</strong></td>
  </tr>
</table>

二维码与 Banner 的替换尺寸见 [`docs/assets/README.md`](docs/assets/README.md)。
